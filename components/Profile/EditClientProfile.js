...
export default function EditClientProfile({ navigation, route }) {
	...
	const handleSubmit = () => {
		...
		fetch('https://bfts-backend.herokuapp.com/clients/updateById', requestOptions)
			.then(async response => {
	...
	return (
		<SafeAreaView style={{ flex: 1, backgroundColor: { BFTS_WHITE } }}>
			<View style={{ alignItems: 'center', justifyContent: 'center', marginVertical: 30 }}>
				<Text style={[{ fontSize: 30, fontFamily: 'Montserrat_500Medium' }]}>Edit Client Profile</Text>
			</View>
			<ScrollView>
				<View style={{ alignItems: 'center', justifyContent: 'center' }}>
					...
					<EditField
						val={currClient.livingSituation}
						submitted={sumbitted}
						multiline={true}
						setter={setLivingSituation}
					/>
					<EditField
						val={currClient.background}
						submitted={sumbitted}
						multiline={true}
			...
			<View
				...
				<TouchableOpacity
					...
					<Text style={[{ fontSize: 18, color: BFTS_WHITE }, { fontFamily: 'Montserrat_700Bold' }]}>
						Submit Changes
					</Text>
				</TouchableOpacity>
			</View>
